...
new_ball = my_program.create_thing('Orange Ball',[Property('is_ball',True),Property('grip_force',3)])
print('\n{0}\n\n'.format(new_ball))

input('Adding A Robot to the State [Enter]')
panda_robot = my_program.create_thing('Panda',[Property('model','Panda'),Property('gripping',False)])
print('\n{0}\n\n'.format(panda_robot))
...
new_primitive = my_program.create_primitive('panda_update_gripper',parameters=['force','spacing','priority'])
...
call_to_primitive = new_primitive.create_call({'force':'grip_force','spacing':'grip_spacing','priority':'priority'})
print('\n{0}\n\n'.format(call_to_primitive))

input('Creating a wrapper for the Primitive [Enter]')
new_action = my_program.create_action('Panda Grasp',parameters=['agent','object'])
new_action.create_definition(name='grip_force',item='object',property='grip_force',fallback=1)
new_action.create_definition(name='grip_spacing',value=None)
new_action.create_definition(name='priority',value=0)
new_action.create_postcondition(thing='agent',property=Property('gripping',True),operation=PropertyOperation.EQUALS)
